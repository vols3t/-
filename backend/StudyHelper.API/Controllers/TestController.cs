    ...
    public async Task<IActionResult> CreateTest([FromBody] TestRequestModel? request)
    {
        ...
        try
        {
            ...
            var systemPrompt =
                ...
                "9. ЯЗЫК ВЫВОДА — ТОЛЬКО РУССКИЙ. Все формулировки, варианты ответов и пояснения пишутся на русском.\n" +
                "   Никакого «mixed-language»: не используй английские слова там, где есть устоявшийся русский эквивалент " +
                "(например: пиши «массив», а не «array»; «класс», а не «class»; «наследование», а не «inheritance»; " +
                "«переменная», а не «variable»; «исключение», а не «exception»).\n" +
                "   Допустимы в исходном виде ТОЛЬКО:\n" +
                "     - имена языков, технологий, протоколов и форматов как термины (C#, Python, JavaScript, HTTP, TCP, JSON, SQL, REST);\n" +
                "     - конкретные ключевые слова, идентификаторы и сигнатуры из кода (public, void, SELECT, async/await, List<T>);\n" +
                "     - имена собственные, бренды и общепринятые аббревиатуры без русского аналога (Linux, Git, IDE).\n" +
                "   Всё остальное — по-русски. Перед выдачей перечитай каждое поле и замени любые лишние английские слова на русские.\n" +
                "10. Без markdown внутри полей: никаких **жирного**, `code`, ссылок и списков — это plain-текст.\n\n" +
            ...
            var userPrompt =
                ...
                "  e) пройдись по каждому полю и убедись, что в нём нет английских слов, у которых есть русский эквивалент " +
                "(см. правило 9 в системе). Если нашёл — замени.\n\n" +
                "Теперь выдай итоговый JSON по схеме выше. Только JSON, только на русском языке.";
